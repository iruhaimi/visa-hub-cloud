...
import { Users, Globe, Award, Headphones, LucideIcon } from 'lucide-react';
import { useSiteSection } from '@/hooks/useSiteContent';
import { useLanguage } from '@/contexts/LanguageContext';

const ICON_MAP: Record<string, LucideIcon> = { Users, Globe, Award, Headphones };
...
export default function StatsSection() {
  ...
  const { data: content } = useSiteSection('home', 'stats');

  const stats = (content?.items || fallbackStats);
  const title = isRTL ? (content?.title || 'أرقام تتحدث عن نجاحنا') : (content?.title_en || 'Numbers That Speak of Our Success');
              ...
              >
                <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-6 md:p-8 border border-white/20 text-center hover:bg-white/15 transition-all duration-300">
                  ...
                  <p className="text-white/70 font-medium">
                    {isRTL ? stat.label : (stat.label_en || stat.label)}
                  </p>
